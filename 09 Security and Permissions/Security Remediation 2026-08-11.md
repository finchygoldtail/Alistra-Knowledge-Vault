...
4. **Inactive and revoked Storage access — High.** Backend Storage authorization now requires explicit `active: true` and rejects disabled Auth accounts. The HTTP API verifies bearer ID tokens with Firebase Admin's canonical revocation checking enabled. Callable endpoints use Firebase's authenticated callable context and do not duplicate the Admin SDK's internal token-timestamp comparison.
...
## Post-release callable session hotfix

The first security release also added a manual comparison between a callable request's `auth_time` claim and the Auth user's `tokensValidAfterTime`. In production this misclassified a freshly signed-in owner session as revoked, blocking map saves and moving them into the browser's recoverable pending-save cache. The cache protected the affected 95 assets from loss.

Commit `4ee8a9d` (`Fix false callable session revocation`) removed only the duplicate timestamp comparison. Explicitly-active tenant membership checks and disabled-account rejection remain enforced for callable storage access, while the HTTP API retains `verifyIdToken(token, true)` revocation verification. The regression suite now prevents the manual comparison from being reintroduced and confirms the HTTP revocation check remains enabled. Verification passed **280/280** application/security tests, application typecheck and the Functions TypeScript build. The map load/save/upsert/delete functions and HTTP API were successfully deployed to Firebase project `fibre-gis-v2` on 2026-08-11.
